...
function makeItem(id: string, type: TimelineItem['type']): TimelineItem {
  switch (type) {
    case 'audio':
      return { id, type, trackId: 'track-1', from: 0, durationInFrames: 30, label: id, src: 'blob:audio' };
    case 'video':
      return { id, type, trackId: 'track-1', from: 0, durationInFrames: 30, label: id, src: 'blob:video' };
    case 'image':
      return { id, type, trackId: 'track-1', from: 0, durationInFrames: 30, label: id, src: 'blob:image' };
    case 'text':
      return { id, type, trackId: 'track-1', from: 0, durationInFrames: 30, label: id, text: id, color: '#fff' };
    case 'shape':
      return { id, type, trackId: 'track-1', from: 0, durationInFrames: 30, label: id, shapeType: 'rectangle', fillColor: '#fff' };
    case 'adjustment':
...
describe('resolveEffectDropTargetIds', () => {
  it('applies to hovered item when it is not part of a multi-selection', () => {
    ...
    expect(resolveEffectDropTargetIds({
