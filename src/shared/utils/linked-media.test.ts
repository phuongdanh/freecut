...
describe('linked-media', () => {
  ...
  it('collects only video ids that own linked audio companions', () => {
    ...
    const standaloneVideo = makeVideoItem({ id: 'video-standalone', linkedGroupId: undefined, mediaId: 'media-2' });

    expect(Array.from(getLinkedVideoIdsWithAudio([pairedVideo, pairedAudio, standaloneVideo]))).toEqual(['video-paired']);
  });
});
